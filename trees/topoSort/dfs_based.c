...
count++;
graph->visit[vertex] = 1;graph->path[vertex] = 1;
while (temp != NULL)
{
...
for(int i=0;i<g->vertices;i++)
{
temp=g->adjLists[i];printf("THE VERTEX ADJACENT TO %c : ",i+65);
while(temp!=NULL)
{
...
Node temp;
int key;
if(k==0)for(int i=0;i<g->vertices;i++)
{
for(int j=0;j<g->vertices;j++)
{
if(i!=g->vertices-1-j)
